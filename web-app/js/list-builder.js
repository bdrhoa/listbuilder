$(function() {
  ...
  {
    $('#leftSelect option:selected').appendTo('#rightSelect').attr({ selected: false }); updateListBuilderState(); this.blur();
  };
  addButton.click(handler).keyup(doNotFireOnTab(handler));
  handler = function(event)
  {
    $('#rightSelect option:selected').appendTo('#leftSelect').attr({ selected: false }); updateListBuilderState(); this.blur();
  };
  removeButton.click(handler).keyup(doNotFireOnTab(handler));
  handler = function(event)
  {
    $('#leftSelect option').appendTo('#rightSelect').attr({ selected: false }); updateListBuilderState(); this.blur();
  };
  addAllButton.click(handler).keyup(doNotFireOnTab(handler));
  handler = function(event)
  {
    $('#rightSelect option').appendTo('#leftSelect').attr({ selected: false }); updateListBuilderState(); this.blur();
  };
